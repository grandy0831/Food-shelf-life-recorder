...
class _MapScreenState extends State<MapScreen> {
                ...
                child: Column(
                  children: [
                    if (_floorRoomsBuilding1 != null || _floorRoomsBuilding2 != null)
                      ..._buildRoomsList(_floorRoomsBuilding1 ?? {}, _floorRoomsBuilding2 ?? {}, favorites) // 传递FavoritesModel
                    else
                      const Center(
                        child: Text(
  ...
  List<Widget> _buildRoomsList(Map<String, List<Room>> floorRoomsBuilding1, Map<String, List<Room>> floorRoomsBuilding2, FavoritesModel favorites) {
    List<Widget> roomCards = [];
    bool hasMatches = false;

    for (var floor in floorRoomsBuilding1.keys) {
      var matchingRooms = floorRoomsBuilding1[floor]!.where((room) => room.roomType.toLowerCase().contains(_searchQuery.toLowerCase())).toList();
      if (matchingRooms.isNotEmpty) {
        hasMatches = true;
        roomCards.addAll(
          matchingRooms
              .map<Widget>((room) => _buildRoomCard(room, floor, favorites)) 
              .toList(),
        );
      }
    }
    ...
    for (var floor in floorRoomsBuilding2.keys) {
      var matchingRooms = floorRoomsBuilding2[floor]!.where((room) => room.roomType.toLowerCase().contains(_searchQuery.toLowerCase())).toList();
      if (matchingRooms.isNotEmpty) {
        hasMatches = true;
        roomCards.addAll(
          matchingRooms
