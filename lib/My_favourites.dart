...
class MyFavoritesScreen extends StatefulWidget {
  const MyFavoritesScreen({Key? key});
...
class RoomsTab extends StatelessWidget {
  const RoomsTab({Key? key});

  @override
  Widget build(BuildContext context) {
    return Consumer<FavoritesModel>(
      builder: (context, favorites, child) {
        final List<String> favoriteRooms = favorites.favoriteRooms;

        return ListView.builder(
          padding: const EdgeInsets.all(8),
          itemCount: favoriteRooms.length,
          itemBuilder: (context, index) {
            final roomName = favoriteRooms[index];

            return Card(
              margin: const EdgeInsets.symmetric(vertical: 8, horizontal: 4),
              child: ListTile(
                title: Text(roomName, style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)), // 只显示房间名称
                trailing: IconButton(
                  icon: Icon(Icons.favorite, color: Colors.redAccent),
                  onPressed: () => favorites.removeRoom(roomName),
                ),
              ),
            );
          },
